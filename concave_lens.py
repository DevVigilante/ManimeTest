...
class Intro(Scene):
    def construct(self):
        animatedBy = Text("Nature of concave lens changing to convex lens", font_size=24)
...
class ConcaveLens(Scene):
    def construct(self):
        heading = Text(
            text="Nature of concave lens changing to convex lens",
            font_size=20
        )
        ...
        lensSurface2.next_to(lensSurface1, RIGHT, buff=0.1)
        ...
        focus.add_updater(
            lambda foc:
                focus.next_to(lensSurface1, RIGHT, buff=focalLengthTracker.get_value())
        )
        focus.next_to(lensSurface1, RIGHT, buff=focalLengthTracker.get_value())
        ...
        value_text = always_redraw(
            lambda: VGroup(
                ...
                Text(f"Focal length = {focalLengthTracker.get_value():.2f} m", font_size=15),
                Text(f"Medium RI = {ri_tracker.get_value():.2}", font_size=15),
                Text(f"Lens RI = {lens_refractive_index:.2}", font_size=15),
                Text(
        ...
        text = Text(
            ...
            And the concave lens will behave as convex lens
            """, font_size=16)
        text.to_corner(DOWN)
        self.play(Write(text))
        self.wait(2)
        ...
        self.wait(1)
        self.play(tracker.animate.set_value(objectDistance), run_time=15)
        self.wait(5)
